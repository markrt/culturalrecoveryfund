...
imd %>% 
  ...
  right_join(money_population_geography) %>% 
  distinct() %>% 
  group_by(Local.Authority,
           IMD...Average.rank) %>% 
  summarise(money_per_head = sum(money_per_head)) %>% 
  filter(money_per_head < 500) %>% 
  ggplot() +
  ...
  scale_x_log10() +
  theme_minimal() +
  labs(x = "£ per head, all CRF sources",
       y = "IMD average rank by Local Authority\n (lower number = more deprived)")
```
